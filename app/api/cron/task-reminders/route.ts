...
import { sendTaskRemindersForWorkspace } from "@/lib/tasks/reminders"
...
/** Daily task-reminder emails. Schedule via cron (Authorization: Bearer CRON_SECRET). */
export async function GET(request: Request) {
  ...
  for (const workspaceId of workspaceIds) {
    const result = await sendTaskRemindersForWorkspace(workspaceId)
